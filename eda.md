...
rnoaa package allows you to access data from online floor\_date -\>
round down
...
How many \_\_\_\_\_\_\_\_\_ exist?

Count month / name observations n\_obs is the variable created to define
the function n()
...
We can use `count()` Does the same thing as above
...
**NEVER** USE BASE R’S `TABLE` function The result of this is not a
dataframe YOu have to pull a variable out of a datafram to use the table
function
...
Other helpful counters n\_obs - number of observations in that month
n\_days - number of unique days in that month (how many dates in that
month are distinct)
...
case\_when is kind of like if/then TRUE \~ "" is just in case some
observations don’t fit into the categories defined
...
Reminder: sometimes your results are easier to read in another format We
can use “pivot wider” because we are doing these calculations in a
dataframe pivot wider is easier to read “knitr::kable()” helps format
info so it is easier to read (creates a table)
...
Rank function - ranks the values Min\_rank ranks coldest as 1 and
increases if you add descending
